...
(defun generate-file (system)
  (let* ((system-dir (asdf:component-pathname (asdf:find-system system)))
	 ...
	 (generator-data (with-open-file (stream asd-gen-pathname)
			   (read stream))))
...
(defun write-asd (system)
  (let* ((*print-case* :downcase)
	...
	(progn (format t "ASD file found at ~A.~%Proceeding.~%" file-pathname)
	       (rename-file file-pathname renamed-pathname)
	       (format t "Renamed file ~A~%to ~A.~%" file-pathname renamed-pathname))
	(progn (format t "ASD file NOT found at ~A.~%Continue?~%" file-pathname)
	       (unless (yes-or-no-p) (return-from write-asd))))
